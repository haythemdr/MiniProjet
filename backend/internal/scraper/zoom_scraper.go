...
func SearchZoomPage(search string, page int) ([]models.Product, bool) {

	var products []models.Product
	seen := make(map[string]bool)

	searchURL := fmt.Sprintf(
		"https://zoom.com.tn/recherche?controller=search&s=%s&page=%d",
	...
	req.Header.Set("Referer", "https://zoom.com.tn/")
	...
	doc.Find(".product-miniature").Each(func(i int, s *goquery.Selection) {
		...
		name := strings.TrimSpace(
			s.Find(".product-name a").Text(),
		)

		productURL, _ := s.Find(".product-name a").Attr("href")
		...
		if img.Length() > 0 {
			...
			if imageURL != "" && strings.HasPrefix(imageURL, "/") {
				imageURL = "https://zoom.com.tn" + imageURL
			}
		}
